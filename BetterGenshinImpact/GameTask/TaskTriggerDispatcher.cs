...
namespace BetterGenshinImpact.GameTask
{
    public class TaskTriggerDispatcher : IDisposable
    {
        ...
        public static TaskTriggerDispatcher Instance()
        {
            if (_instance == null)
            {
                throw new Exception("请先在启动页启动耕地机，如果已经启动请重启");
            }
        ...
        public void Tick(object? sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (GameCapture == null || !GameCapture.IsCapturing)
                {
                    ...
                    {
                        _logger.LogInformation("游戏已退出，耕地机 自动停止截图器");
                    }
                ...
                if (!active)
                {
                    ...
                    if (!TaskContext.Instance().Config.MaskWindowConfig.UseSubform)
                    {
                        ...
                        if (pName != "Assistant" && pName != "YuanShen" && pName != "GenshinImpact" && pName != "Genshin Impact Cloud Game")
                        {
                            maskWindow.Invoke(() => { maskWindow.Hide(); });
                        }
                    }
        ...
        {
            RouteExecutor.TpPrev(false);
        }

        public static void QmTpNext()
        {
